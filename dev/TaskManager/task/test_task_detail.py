...
class TaskDetailTest(APITestCase):
    ...
    def test_get_task_detail_superUser(self):
        """Test super user can get task-detail"""
    ...
    def test_delete_task_superUser(self):
        """Test superuser can delete task."""
        self.client.force_authenticate(user=self.superuser)
        response = self.client.delete(self.url)
    ...
    def test_update_task_worker(self):
        """Test worker in proj can update task-detail"""
        self.client.force_authenticate(user=self.user_worker)
    ...
    def test_update_task_superUser(self):
        """Test superuser can update task-detail"""
        self.client.force_authenticate(user=self.superuser)
        payload = {'name': 'Updated Task'}
        response = self.client.patch(self.url, payload)
    ...
    def test_update_task_worker_not_in_proj(self):
        """Test worker not in proj can not update task-detail"""
        self.client.force_authenticate(user=self.user_worker2)
        response = self.client.patch(self.url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_update_task_proj_leader(self):
        """Test Proj leader can update task within proj it is in"""
        self.client.force_authenticate(user=self.user_proj_leader)
        payload = {'name': 'Updated Task'}
        response = self.client.patch(self.url, payload)
    ...
    def test_update_task_org_leader(self):
        """Test Org leader can not update task within proj it is in"""
        self.client.force_authenticate(user=self.user_org_leader)
        response = self.client.patch(self.url)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
